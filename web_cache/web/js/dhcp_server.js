...
function preSubmit() {
	...
	var rel = /^[0-9]{1,}$/;
	if (sip == "" || eip == "") {
		showErrMsg("msg-err", _("Please enter a value in the IP Range field."));
		return;
	}
	if (!rel.test(sip) || !rel.test(eip)) {
		showErrMsg("msg-err", "Only digit is allowed!");
		return;
	}

	if (parseInt(sip, 10) > parseInt(eip, 10)) {
		showErrMsg("msg-err", _("The start IP should not be greater than the end IP."));
		return;
	}
	if (parseInt(sip, 10) <= 0 || parseInt(eip, 10) >= 255) {
		showErrMsg("msg-err", _("The last number of IP Range should be a value between 1 and 254."));
		return;
	}
