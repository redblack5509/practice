...
$(function () {
	...
	$('#SYSOPS').initPassword("", false, false);
	$('#SYSPS').initPassword("", false, false);
	$('#SYSPS2').initPassword("", false, false);
...
system = {
	...
	password: function () {
		...
		if ((newPw == oldPw) && ((!$('#old_pwd').hasClass('none')) || (oldPw === ""))) {
			showErrMsg("msg-err", _("The new password cannot be the same as the old one."));
			G.index = 1;
			return false;
		}
		...
		if (newPw != confirmPw) {
			showErrMsg("msg-err", _("New password and confirm password mismatch!"));
			G.index = 1;
			return false;
		}

		if (newPw == "" && G.remoteEn == "1") {
			showErrMsg("msg-err", _("No login password will produce security risk when the Remote Management feature is enabled. Please go to \"Advanced Settings\" to disable the Remote Management feature."));
			G.index = 1;
			return false;
		}
...
function init() {
	...
	if (msg == "1") {
		//if (!$('#old_pwd').hasClass('none')) {
			$("#msg-err").html(_("Wrong Old Password!"));
			top.$(".main-dailog").removeClass("none");
		//}
	} else {
		$("#msg-err").html("&nbsp;");
...
function checkCloudServer() {
	$.getJSON("goform/cloud?module=getInfo&rand=" + new Date().toTimeString(), function (obj) {
		if (obj.enable == 0) {
			if (!obj.password) {
				//无密码需要自动生成密码
				obj.password = hex_md5(randomString());
			}
			var subObj = {
				"enable": 1,
