...
function rebooting(time) {
	...
	if (pc <= 100) {
		...
		clearTimeout(rebootTime);
		pc = 1;
		if (ipaddress != "" && (/^([1-9]|[1-9]\d|1\d\d|2[0-1]\d|22[0-3])\.(([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.){2}([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])$/).test(ipaddress) == true || ipaddress == "pisnetwifi.com") {
