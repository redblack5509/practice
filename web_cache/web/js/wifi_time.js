...
function initValue(obj) {
	...
	if (obj.wl_mode != "ap") {
		showErrMsg("msg-err",_("The WiFi Schedule is not available while the Wireless Repeating is enabled. Please go to Wireless Settings part to disable the Wireless Repeating first."),true);
	...
	if (schedWifiEnable == 1) {
		...
		if (initObj.powerSaveTime) {
			...
			if (isTimeOverlaping(powerSaveTimeStart, powerSaveTimeEnd, parseInt(startHour+""+startMin, 10), parseInt(endHour+""+endMin, 10))) {
				//重叠
				if (!window.confirm(_("The time period you set up in Sleeping Mode (%s) overlaps with that in WiFi Schedule (%s). During the overlapping time, the settings in WiFi Schedule will be ineffective. Are you sure to save the settings?", [initObj.powerSaveTime, time]))) {
					return false;
				}
			}
		}
