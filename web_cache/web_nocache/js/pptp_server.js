...
function addList(rowDataObj) {
	...
	str += "<td><input data-role='password' type='password' id='password" + listNum + "' class='input-small' maxLength='32' style='width:80px;'></td>";
	...
	$('[data-role="password"]').initPassword(_(""), false, false);
		...
		custom: function () {
			...
			if ($("#serverEn").val() == "1") {
				...
				if (serverIp != "" && checkIpInSameSegment(startIp, lanMask, serverIp, "255.255.255.0")) {
					return _("The start IP address of address pool conflicts with the IP address of dual-access server (%s). Please make a change.",[serverIp]);
				}

				if (vlan2Ip != "" && checkIpInSameSegment(startIp, lanMask, vlan2Ip, vlan2Mask)) {
					return _("The start IP address of address pool conflicts with the dual-access IP address (%s). Please make a change.",[vlan2Ip]);
				}
