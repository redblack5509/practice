...
function changeBridge() {
	...
	if (className == "btn-off") {
		...
		if (initObj.relayCloseFlag == "2.4G_closed") {
			showErrMsg("msg-err", _("Before you disable Wireless Repeating at 2.4G band, please go to page WiFi Name & Password and enable 2.4GHz."));
			return;
		} else if (initObj.relayCloseFlag == "5G_closed") {
			showErrMsg("msg-err", _("Before you disable Wireless Repeating at 5G band, please go to page WiFi Name & Password and enable 5GHz."));
			return;
		}
...
function changeScanSsid() {
	var ssid = $("#apSsid").val();

	if(ssid == -3) { //手动设置
		if (initObj.wrlEn == "0" && $("#wifi_chkHz").val() == 0) {
			showErrMsg("msg-err", _("Please go to page WiFi Name & Password and enable 2.4GHz."));
			return false;
		} else if (initObj.wrlEn_5g == "0" && $("#wifi_chkHz").val() == 1) {
			showErrMsg("msg-err", _("Please go to page WiFi Name & Password and enable 5GHz."));
			return false;
		}
		return true;
	}
	
	if(initObj.relayCloseFlag == "2.4G_closed" || initObj.relayCloseFlag == "5G_closed") {
		if(!scanFlag) {
			if(initObj.relayCloseFlag == "2.4G_closed") {
				showErrMsg("msg-err", _("Please go to page WiFi Name & Password and enable 2.4GHz."));
			} else {
...
function preSubmit() {
	...
	if(initObj.wl_enable == "1"){
		if (reboot&&!confirm(_("The router will reboot to activate your settings, are you sure to reboot now?"))) {
			return false;
		} else {
...
function callback(str, reboot, wlMode) {
	...
	if (num == 0) {
		...
		if (reboot) {
			...
			top.$.progress.showPro((wlMode == "apclient" || wlMode == "wisp"? "apclient": "reboot"), "", (wlMode == "apclient"? "pisnetwifi.com": ""));
