...
var initObj = {};
$(function () {
	if (+top.CONFIG_IPTV_STB == 3) {  //宏控制lan口
		$("#stbEn .text-warning").html(_("Tip: Please connect the Set-top Box to the Router's LAN3/IPTV port."));
	}
...
function initValue(obj) {
	...
	if (obj.wl_mode != "ap") {
		showErrMsg("msg-err", _("The Wireless Repeating is enabled. Please go to Wireless Settings to disable Wireless Repeating first."), true);
...
function preSubmit() {
	...
	if (reboot && !confirm(_("Please reboot the device if you do any change to the Set-top Box. Are you sure to reboot the device?"))) {
		return false;
	}
