...
function preSubmit() {
	...
	if (ledType == "time") {
		...
		if (initObj.powerSaveTime) {
			...
			if (isTimeOverlaping(powerSaveTimeStart, powerSaveTimeEnd, parseInt(startHour+""+startMin, 10), parseInt(endHour+""+endMin, 10))) {
				//重叠
				if (!window.confirm(_("The time period you set up in Sleeping Mode (%s) overlaps with that in LED Control (%s). During the overlapping time, the settings in LED Control will be ineffective. Save the settings?", [initObj.powerSaveTime, time]))) {
					return false;
				}
			}
		}
	...
	/*if (initObj.timeUp == "0" && $("[name='ledType']:checked").val() == "time") {
		if (!confirm(_("The LED Schedule takes effect when the system time synchronizes the Internet time."))) {
			return false;
		}
	}*/
