        ...
        custom: function () {
            ...
            if ((netWanType === "3") || (netWanType === "4")) {
                ...
                if (checkIpInSameSegment(vpnServer, lanMask, lanIp, lanMask)) {
                    inputMsg = _("%s and %s (%s) should not be in the same network segment.", [_("Server IP"), _("LAN IP"), lanIp]);
                }
            }
...
function getInitData() {
    ...
    $.getJSON("goform/fast_setting_get" + "?" + Math.random(), function (obj) {
        ...
        if ((obj.lanIp !== host) && ('pisnetwifi.com' !== host)) {
            location.host = 'pisnetwifi.com';
        }
    });
...
function nextSet() {
    ...
    if (!$("#net_find").hasClass("none")) {
        ...
        if (getStrByteNum(ssid) > 29) {
            showErrMsg("message-ssid", _("The WiFi name should be within %s characters.", [29]));
            return;
        }
...
function showFinish() {
    ...
    if (G.wanStatus == "7") {
        //已联网，
        $(".loadding-ok img").attr("src", "./img/ok_connected.png?44f58592b28037b03ec5c3a7ddacfbfa")
        ...
        if (G.wanStatus == 7) {
            ...
            $(".loadding-ok img").attr("src", "./img/ok.png?44f58592b28037b03ec5c3a7ddacfbfa")
