	...
	$.dialog = (function () {
		...
		function Dialog(options) {
			...
			if ($.type(options) === 'object') {
				...
				this.options = $.extend(defaults, {content: options});
			}
		}	
...
valid = {
	...
	byteLen: function (str, min, max) {
		...
		if(totalLength < min || totalLength > max) {	
			return _('String length range is: %s - %s byte', [min, max]);	
		}
	},
	...
	routeCheck: {
        all: function (str) {
            ...
            if(!(/^([1-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.){2}([0-9]|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])$/).test(str)) {
                return _("Please enter a network segment IP.");
            }
        },
        ...
        specific: function (str) {
            ...
            if (ipArr[0] > 223) {
                return _("First input %s cannot be greater than 223.", [ipArr[0]]);
            }
        }
    }
...
$.extend($.reasyui.b28n, {
	...
	"Can't input: '%s' and blank space": "不能输入: ‘%s’ 和空格",
	...
	"First input %s cannot be greater than 223.": "以%s开始的地址无效，请指定一个1到223之间的值。",
	"First input %s less than 223.": "以 %s 开始的地址无效，请指定一个223到255之间的值。",
	"Please input a valid domain name.": "请输入有效的域名"
});
...
$.include({
	addValidateTip: function (str) {
		...
		return this.each(function () {
			...
			$tipElem = $(tipElem).css({"position":"absolute","width":"0", "height": "0"});
...
$.extend($.validate.valid, {
	...
	fh1901Ascii: function(str, min, max) {
		...
		if (!asciiTestStr) {
			//通过了一般ascii测试
			if(/[\\\~;'&\s"]/.test(str)) {
				return  _("Can't input: '%s' and blank space", ["\~\;\'\&\""]);
			}
		} else {
			return asciiTestStr;
		}
