...
(function (win, doc) {
	...
	var Inputs = {
		...
		createTextInput: function (elem) {
			...
			$newField = $(newField);
			//password输入框focus时
			$elem.parent().find(".invis-eyes").on("click", function () {
				var thisVal = elem.value;
				$(this).addClass("none");
				$(this).parent().find(".vis-eyes").removeClass("none");
				//if (thisVal !== "") {
			...
			//text输入框blur
			$newField.parent().find(".vis-eyes").on("click", function () { //IE下的text输入框
					var $this = $(this);

					$(this).addClass("none");
					$(this).parent().find(".invis-eyes").removeClass("none");
					$newField.hide();
					$newField.removeValidateTip(true);
					$elem.show();
					$elem.focus();
					if ($newField[0].value !== "") {
						elem.value = $newField[0].value;
		...
		toTextType: function (elem) {
			...
			if (Inputs.supChangeType) {
				...
				//点击闭合眼睛，密文显示
				$elem.parent().find(".vis-eyes").on("click", function () {
		...
		addPlaceholder: function (elem, placeholderText) {
			...
			function createPlaceholderElem(elem) {
				...
				$(ret).on('click', function () {
					//显示明文
					if ($(elem).parent().parent().find(".invis-eyes").hasClass("none")) {
						$(elem).next().hide();
						$(elem).show();
						elem.focus();
					} else {
			...
			function showPlaceholder(node) {
				if (node.value === "") {
					if (node.placeholdered !== 'ok') {
						placehodereElem = createPlaceholderElem(elem, node);
...
(function (document) {
	...
	var Inputselect = {
		...
		create: function (elem, obj) {
			...
			dropBtnStr = '<div class="btn-group"><button type="button"' + toggleAble + ' class="dropdown-toggle toggle btn btn-sm"><span class="caret"></span></button></div>';
	...
	valid = {
		...
		num: function (str, min, max) {
			...
			if (min && max) {
				if (parseInt(str, 10) < min || parseInt(str, 10) > max) {
					...
					return _("Input range is: %s~%s", [min, max]);
				}
			}
		},
		...
		mac: {
			...
			specific: function (str) {
				...
				if (str === "00:00:00:00:00:00") {
					return _('MAC address can not be 00:00:00:00:00:00');
				}
			}
		},
		...
		ip: {
			...
			specific: function (str) {
				...
				if (ipArr[0] > 223) {
					return _("The value %s is invalid, please enter value between 1-223.", [ipHead]);
				}
			}
		},
		...
		username: function (str) {
			if (!(/^\w{1,}$/).test(str)) {
				return _("Please input a valid user name.");
			}
		},
		...
		ssid: function (str) {
			...
			if (length > 32) {
				return _("The length should not be greater than 32 characters.");
			}
		},
		...
		syspwd: {
			all: function (str, min, max) {
				var ret = this.specific(str, min, max);
				return ret;
			},
			specific: function (str, min, max) {
				var ret;
				if (!(/^[ -~]+$/g).test(str)) {
					return _("Must be ASCII.");
				}
				...
				if (min && max) {
					ret = $.validate.valid.len(str, min, max);
					if (ret) {
						return ret;
					}
				}
				...
				if (str.indexOf(" ") != -1) {
					return _("'%s' is not allowed", ['&lt;Space&gt;']);
				}
			}
		},
		...
		domain: function (str) {
			if (/^[-_~\|\#\?&\\\/\.%0-9a-z]+$/ig.test(str)) {
				if (/.+\..+/ig.test(str) || str == "localhost") {
					...
					return _("Please input a valid domain name.");
				}
			} else {
				return _("Please input a valid domain name.");
			}
		...
		ping: function (str) {
			...
			if (ip && domain) {
				return _("Please input a valid IP address OR domain name.");
			}
