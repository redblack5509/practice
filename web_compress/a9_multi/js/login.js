(function () {
	var LoginView = function (encode) {
		...
		this.successCallback = function (str) {
			...
			if (num == 0) {
				...
				that.showInvalidError(_("Error – the admin password is incorrect."));
			}
		};
	...
	function LoginLogic(view, submit) {
		var that = this;
		this.init = function () {
			var errStr = location.href.split("?")[1] || 0,
				codeArr, errCode;
			if (!errStr) {
				$("#loginWrap").addClass("fly-in");
			} else {
				codeArr = errStr.split("&");
				errCode = codeArr[0];
				switch (errCode) {
				case "1":
					//返回1时提示错误
					...
					//当存在错误时间时
					if (codeArr[1] && codeArr[1] != "0") {
						view.showInvalidError(_("Error – too many failed attempts, <br /> please try again in %s minute(s).", [codeArr[1]]));
					} else {
						view.showInvalidError(_("Error – the admin password is incorrect."));
					}
					break;
				case "2": //有登录密码&&登录用户已满4个
					view.showInvalidError(_("Login users have been up to 4!"));
					$("#loginWrap").addClass("fly-in");
					break;
			...
			this.validate = $.validate({
				custom: function () {
					...
					if (!checksValid(password)) {
						return _("Error - please enter the admin password.");
					}
				},
